<script lang="ts">
	...
	import { Moon, Sun, HelpCircle, Bell, ChevronDown, LogOut, User, Settings, CalendarDays, History, Shield } from 'lucide-svelte';
...
>
	<!-- Left: branding on mobile/tablet, hidden on desktop -->
	<div class="flex items-center gap-2">
		<!-- Branding — mobile/tablet only -->
		<div class="flex items-center gap-2 lg:hidden">
			<div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-purple-500 to-indigo-600 shadow-md">
				<img src={logo} alt="CHTM Logo" class="h-5 w-5 object-contain" />
			</div>
			<div class="leading-tight">
				<p class="text-xs font-bold text-gray-900">Superadmin</p>
				<p class="text-[10px] text-gray-400">System Control</p>
			</div>
		</div>
	...
	<div class="ml-auto flex items-center gap-0.5">
		...
		<div class="relative" data-topnav-dropdown>
			...
			>
				...
				{#if unreadCount > 0}
					<span class="absolute -right-1 -top-1 inline-flex min-h-[18px] min-w-[18px] items-center justify-center rounded-full bg-purple-600 px-1 text-[10px] font-bold text-white ring-2 ring-white">
		...
		{#if $user}
			<div class="relative" data-topnav-dropdown>
				...
				>
					<div class="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-purple-100 to-indigo-50 border-2 border-purple-200 text-purple-700 text-xs font-semibold shadow-sm">
