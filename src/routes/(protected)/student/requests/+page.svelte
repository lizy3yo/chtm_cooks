<script lang="ts">
	...
	const totalPages = $derived(
		Math.ceil(
			filteredRequests.length / (requestViewMode === 'card' ? PAGE_SIZE_CARD : PAGE_SIZE_LIST)
		)
	);
			...
			<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
				<div class="relative flex-1">
					...
					<input
						...
						class="block h-10 w-full rounded-xl border border-gray-300 bg-white py-2 pr-3 pl-9 text-sm placeholder-gray-400 shadow-sm focus:border-pink-500 focus:ring-2 focus:ring-pink-100 focus:outline-none"
					/>
				</div>
			...
			<div class="mt-6">
				...
				>
					<div class="flex min-w-0 items-center gap-2">
						...
						<span
							class="hidden rounded-full bg-white px-2.5 py-1 text-xs font-medium text-gray-500 ring-1 ring-gray-200 sm:inline-flex"
						>
					...
					<div class="flex flex-wrap items-center justify-end gap-2">
						<div class="flex overflow-hidden rounded-lg border border-gray-300">
							<button
								...
								class="flex h-10 w-10 items-center justify-center text-sm transition-colors {requestViewMode ===
								'list'
							...
							<button
								...
								class="flex h-10 w-10 items-center justify-center border-l border-gray-300 text-sm transition-colors {requestViewMode ===
								'card'
									? 'bg-pink-100 text-pink-700'
									: 'bg-white text-gray-600 hover:bg-gray-50'}"
							>
						...
						<a
							...
							class="inline-flex h-10 shrink-0 items-center gap-1.5 rounded-xl bg-pink-600 px-3 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-pink-700 sm:px-4 sm:text-sm"
						>
							<Plus size={13} />
							New
						</a>
				...
				{#if loading && requests.length === 0}
					<div class="space-y-4">
						{#each Array(3) as _, i}
							<div
								class="overflow-hidden rounded-xl border-l-4 border-gray-200 bg-white shadow-sm ring-1 ring-gray-200"
							>
							...
							<div style="min-height: 600px;">
								<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" style="align-content: start;">
									{#each paginatedRequests as request}
										...
										<div
											class="cursor-pointer overflow-hidden rounded-xl border-l-4 bg-white shadow-sm ring-1 ring-gray-200 transition-all hover:shadow-md {getStatusBorderColor(
												request.status
											...
											<div class="p-4 sm:p-5">
												...
												<div class="mb-3 flex items-start justify-between gap-3">
													<div class="flex min-w-0 flex-1 flex-col gap-1">
														<span class="truncate font-mono text-sm font-bold text-gray-900"
															>{request.id}</span
														>
												...
												<div class="mt-3">
													<div class="flex flex-wrap gap-1.5">
														{#each request.items.slice(0, 3) as item}
															...
															<span
																class="inline-flex items-center gap-1.5 rounded-md border border-gray-200 bg-gray-50 px-2 py-1 text-[11px] font-medium text-gray-700"
															>
																{#if pic}
																	<img
																		src={pic}
																...
																{:else}
																	...
																	>
																{/if}
																<span class="max-w-[80px] truncate">{item.name}</span>
															</span>
														{/each}
												...
												<div class="flex items-center gap-1.5 self-end sm:self-auto">
													...
													{#if request.status === 'pending'}
														<button
															onclick={(e) => {
																e.stopPropagation();
																requestCancelConfirmation(request);
															}}
						...
						{:else}
							...
							<div style="min-height: 600px;">
								<div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
									<div
										class="hidden border-b border-gray-200 bg-gray-50 px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase md:grid md:grid-cols-[32px_0.8fr_2.2fr_0.8fr_100px] md:items-center md:gap-4"
									>
									...
									<div class="divide-y divide-gray-100">
										{#each paginatedRequests as request, i}
											...
											<!-- svelte-ignore a11y_no_static_element_interactions -->
											<div
												class="grid cursor-pointer gap-3 p-4 transition-colors md:grid-cols-[32px_0.8fr_2.2fr_0.8fr_100px] md:items-start md:gap-4 {highlightedRequestId ===
												request.rawId
											...
											>
												...
												<div class="min-w-0 pt-0.5">
													<p
														class="truncate font-mono text-xs font-bold tracking-wider text-gray-900"
													>
														{request.id}
													</p>
												...
												<div class="min-w-0 pt-0.5">
													...
													{:else if request.status === 'pending-return'}
														<p
															class="mt-1.5 flex items-center gap-1 text-[11px] font-medium text-orange-600"
														>
						...
						<div class="flex shrink-0 items-center gap-1.5 sm:gap-2">
							{#if ['approved', 'ready', 'picked-up', 'pending-return'].includes(selectedRequest.status)}
								<button
									...
									class="rounded-xl p-2 text-pink-600 transition-all hover:bg-pink-50 active:scale-95 sm:p-2.5"
									title="View QR Code"
								>
...
{#if showQrModal && selectedRequest && qrDataUrl}
	<div class="fixed inset-0 z-50 overflow-y-auto">
		<div
			...
			tabindex="0"
			onkeydown={(e) => {
				if (e.key === 'Enter' || e.key === ' ' || e.key === 'Spacebar') showQrModal = false;
			}}
			aria-label="Close QR modal"
		></div>
		<div class="flex min-h-full items-end justify-center sm:items-center sm:p-4">
			<div
				class="animate-scaleIn relative w-full max-w-md overflow-hidden rounded-t-3xl bg-white shadow-2xl ring-1 ring-black/5 sm:rounded-3xl"
			>
				<!-- Header -->
				<div
					class="relative border-b border-gray-100 bg-linear-to-br from-pink-50 via-white to-purple-50 px-5 py-5 sm:px-6 sm:py-6"
				>
					<div>
						<div
							class="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1.5 shadow-sm ring-1 ring-gray-100"
						>
							<QrCode size={16} class="text-pink-600" strokeWidth={2.5} />
							<span class="text-sm font-semibold text-gray-700">QR Code</span>
						</div>
						<h3 class="mt-3 text-lg font-bold text-gray-900 sm:text-xl">Scan to Process</h3>
						<p class="mt-1 text-xs text-gray-600 sm:text-sm">Present this code to the custodian</p>
					</div>
				</div>

				<!-- Content -->
				<div class="max-h-[70vh] overflow-y-auto px-5 py-6 sm:px-6 sm:py-8">
					<div class="flex flex-col items-center space-y-5 sm:space-y-6">
						<!-- QR Code with decorative frame -->
						<div class="relative">
							<!-- Decorative corners -->
							<div
								class="absolute -top-2 -left-2 h-5 w-5 rounded-tl-lg border-t-3 border-l-3 border-pink-500 sm:h-6 sm:w-6"
							></div>
							<div
								class="absolute -top-2 -right-2 h-5 w-5 rounded-tr-lg border-t-3 border-r-3 border-pink-500 sm:h-6 sm:w-6"
							></div>
							<div
								class="absolute -bottom-2 -left-2 h-5 w-5 rounded-bl-lg border-b-3 border-l-3 border-pink-500 sm:h-6 sm:w-6"
							></div>
							<div
								class="absolute -right-2 -bottom-2 h-5 w-5 rounded-br-lg border-r-3 border-b-3 border-pink-500 sm:h-6 sm:w-6"
							></div>

							<!-- QR Code -->
							<div class="rounded-2xl bg-white p-4 shadow-lg ring-1 ring-gray-100 sm:p-6">
						...
						<!-- Request ID Badge -->
						<div
							class="w-full rounded-xl bg-linear-to-br from-gray-50 to-gray-100/50 p-3 text-center ring-1 ring-gray-200/50 sm:p-4"
						>
							<p
								class="mb-1 text-[10px] font-semibold tracking-widest text-gray-500 uppercase sm:mb-1.5 sm:text-xs"
							>
								Request ID
							</p>
							<p class="font-mono text-xl font-bold tracking-wider text-gray-900 sm:text-2xl">
								{selectedRequest.id}
							</p>
						</div>
						...
						<!-- Status Badge -->
						<div
							class="inline-flex items-center gap-2 rounded-full px-3 py-1.5 sm:px-4 sm:py-2 {getStatusColor(
						...
						<!-- Instructions Card -->
						<div
							class="w-full rounded-xl border border-blue-200 bg-linear-to-br from-blue-50 to-blue-100/30 p-3 sm:p-4"
						>
							<div class="flex gap-2.5 sm:gap-3">
								<div
									class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-500 sm:h-8 sm:w-8"
								>
									<Info size={14} class="text-white sm:hidden" />
									<Info size={16} class="hidden text-white sm:block" />
								</div>
								<div class="min-w-0 flex-1">
									<p class="text-xs leading-relaxed font-medium text-blue-900 sm:text-sm">
				...
				<!-- Footer -->
				<div
					class="safe-area-bottom border-t border-gray-100 bg-linear-to-br from-gray-50 to-white px-5 py-4 sm:px-6 sm:py-5"
				>
					<button
						onclick={() => (showQrModal = false)}
