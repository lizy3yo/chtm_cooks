	...
	function getApprovalTimeline(request: any) {
		...
		if (request.status === 'returned') {
			timeline.push({
				...
				by: request.instructor?.fullName || 'Primary Admin'
			});
			timeline.push({
			...
			timeline.push({
				step: 'Returned',
				status: 'completed',
				date: request.returnedAt || request.createdAt,
				by: 'Student'
			});
			...
			timeline.push({
				...
				status: 'rejected',
				date: request.createdAt,
				by: request.instructor?.fullName || 'Instructor'
			});
		}
							...
							<button
								onclick={() =>
									openDetailModal({
										...req,
										rawId: req.id,
										id: formatRequestCode(req.id),
										items: req.items,
										status: uiStatus,
										requestDate: req.createdAt,
										purpose: req.purpose,
										instructor: req.instructor?.fullName || 'N/A',
			...
			>
				...
				<div class="sticky top-0 z-10 border-b border-gray-200 bg-white px-6 py-5 sm:px-8">
					<div class="flex items-start justify-between gap-3">
						<div class="flex min-w-0 flex-1 items-start gap-3">
							<div
								class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-linear-to-br from-pink-500 to-pink-600 shadow-lg shadow-pink-500/30"
							>
								<ClipboardList size={24} class="text-white" strokeWidth={2.5} />
							</div>
							<div class="min-w-0 flex-1">
								<h2 class="text-xl font-bold text-gray-900">Request Details</h2>
								<p class="mt-0.5 font-mono text-sm font-semibold text-pink-600">
									{selectedRequest.id}
								</p>
								<div
									class="mt-2 inline-flex items-center gap-2 rounded-full px-3 py-1.5 {getStatusColor(
				...
				<div class="max-h-[70vh] overflow-y-auto px-6 py-6 sm:px-8 sm:py-8">
					<div class="space-y-8">
						...
						<div>
							<div
								class="rounded-2xl border border-gray-200 bg-linear-to-br from-white to-gray-50 p-5"
							>
								<div class="relative">
										...
										{#each getApprovalTimeline(selectedRequest) as step}
											...
											<div class="flex flex-1 flex-col items-center">
												<div class="relative mb-2 flex items-center justify-center">
													<div
														class="flex h-12 w-12 items-center justify-center rounded-full border-2 bg-white {isCompleted
															? 'border-pink-600'
															: isCancelled
																? 'border-slate-400'
																: isRejected
													...
													>
														{#if step.step === 'Request Submitted'}
															<FileCheck
																size={20}
																class={isCompleted ? 'text-pink-600' : 'text-gray-400'}
															/>
												...
												<div class="min-w-0 text-center">
													<p class="line-clamp-2 text-xs leading-tight font-semibold text-gray-900">
														{step.step}
													</p>
													<p class="mt-0.5 line-clamp-1 text-xs text-gray-500">{step.by}</p>
													<p
								...
								<div
									class="mt-4 flex flex-wrap justify-center gap-3 border-t border-gray-200 pt-3 text-xs"
								>
									<div class="flex items-center gap-1.5">
						...
						<div>
							...
							<div class="grid grid-cols-2 gap-4">
								<div
									class="group rounded-xl border border-gray-200 bg-linear-to-br from-white to-gray-50 p-4 transition-all hover:border-pink-200 hover:shadow-md"
								>
								...
								>
									<div class="mb-2 flex items-center gap-2">
										...
										<p class="text-xs font-bold tracking-wider text-gray-500 uppercase">Purpose</p>
									</div>
									<p class="line-clamp-2 text-base font-bold text-gray-900">
										{selectedRequest.purpose}
									</p>
								</div>
								<div
									class="group col-span-2 rounded-xl border border-gray-200 bg-linear-to-br from-white to-gray-50 p-4 transition-all hover:border-pink-200 hover:shadow-md"
								>
									<div class="mb-2 flex items-center gap-2">
