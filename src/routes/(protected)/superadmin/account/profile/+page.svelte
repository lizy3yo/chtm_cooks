...
<div class="space-y-6">
	<div>
		<div>
			<h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">Administrator Profile</h1>
			<p class="mt-1 text-sm text-gray-500">Manage your account information and keep your credentials secure.</p>
		</div>
	</div>
					...
					<div class="group relative h-48 w-48 shrink-0">
						{#if profile?.profilePhotoUrl}
							<img
								...
								alt={(profile && `${profile.firstName ?? ''} ${profile.lastName ?? ''}`.trim()) || 'Administrator'}
			...
			<div class="space-y-6 lg:col-span-2">
				<section bind:this={personalInfoCard} class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
					...
					<div class="mt-5 grid gap-4 sm:grid-cols-2">
						...
						<Input id="email" label="Email" value={email} disabled helperText="Email cannot be changed from this page." class="sm:col-span-2" />
					</div>

					<div class="mt-6 rounded-lg border border-pink-100 bg-pink-50 px-4 py-3 text-sm text-pink-700">
						Account details are managed at the system level. Contact your system owner to request changes.
					</div>
				</section>
