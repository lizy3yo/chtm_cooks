<script lang="ts">
	...
	import { Search, Plus, Upload, Download, Info, Edit, Trash2, UserCheck, UserX, MoreVertical, X, RefreshCw, Eye, KeyRound, Wifi, WifiOff } from 'lucide-svelte';
	import { usersAPI, type UserResponse, type UserRole, type ProfilePhotoUpdatedEvent } from '$lib/api/users';
	...
	let stats = $state({ total: 0, active: 0, inactive: 0, newThisMonth: 0, students: 0, instructors: 0, custodians: 0 });

	// ─── Create user form ────────────────────────────────────────────────────────
	let createForm = $state({ firstName: '', lastName: '', email: '', password: '', role: '' as UserRole | '', yearLevel: '', block: '' });
	let createLoading = $state(false);
	let createErrors = $state<Record<string, string>>({});

	// ─── Edit user modal ─────────────────────────────────────────────────────────
	let editUser = $state<UserResponse | null>(null);
	let editForm = $state({ firstName: '', lastName: '', role: '' as UserRole, yearLevel: '', block: '', isActive: true });
	...
	async function loadUsers(showLoader = true) {
		...
		try {
			...
			const res = await usersAPI.getAll({ role, search: searchQuery || undefined, page: pagination.page, limit: pagination.limit, forceRefresh: true });
			let list = res.users;
	...
	function onFilterChange() { pagination.page = 1; loadUsers(); }
	...
	function validateCreate(): boolean {
		...
		if (createForm.role === 'student' && !createForm.yearLevel) errs.yearLevel = 'Required for students';
	...
	async function handleCreateUser() {
		...
		try {
			...
			toastStore.success(`${createForm.firstName} ${createForm.lastName} created successfully`, 'User Created');
			createForm = { firstName: '', lastName: '', email: '', password: '', role: '', yearLevel: '', block: '' };
			createErrors = {};
			activeTab = 'all';
			await loadUsers();
			await loadStats();
	...
	function openEdit(user: UserResponse) {
		editUser = user;
		editForm = { firstName: user.firstName, lastName: user.lastName, role: user.role, yearLevel: user.yearLevel || '', block: user.block || '', isActive: user.isActive };
		openDropdown = null;
	}
	...
	async function handleToggleActive(user: UserResponse) {
		...
		try {
			...
			toastStore.success(`${user.firstName} ${user.isActive ? 'deactivated' : 'reactivated'} successfully`);
			await loadUsers();
			await loadStats();
		} catch (e: any) {
	...
	async function handleDelete(user: UserResponse) {
		...
		const confirmed = await confirmStore.danger(
			...
			'Delete User', 'Delete Permanently'
		);
		if (!confirmed) return;
		try {
	...
	function handleFileSelect(file: File) {
		...
		reader.onload = (e) => {
			...
			if (lines.length < 2) { bulkErrors = ['File must have a header row and at least one data row.']; return; }
			const headers = parseCsvLine(lines[0]).map(h => h.toLowerCase().replace(/\s/g, '_'));
			const rows: Array<Record<string, string>> = [];
			const errs: string[] = [];
			for (let i = 1; i < lines.length; i++) {
				const vals = parseCsvLine(lines[i]);
				if (vals.length < headers.length) { errs.push(`Row ${i + 1}: insufficient columns`); continue; }
				const row: Record<string, string> = {};
				headers.forEach((h, idx) => { row[h] = vals[idx] || ''; });
				if (!row.email) { errs.push(`Row ${i + 1}: email is required`); continue; }
				if (!row.first_name) { errs.push(`Row ${i + 1}: first_name is required`); continue; }
				if (!row.last_name) { errs.push(`Row ${i + 1}: last_name is required`); continue; }
				rows.push(row);
			}
			bulkPreview = rows;
			bulkErrors = errs;
	...
	function statusBadge(active: boolean) {
		return active ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-gray-100 text-gray-500 border border-gray-200';
	}
	...
	const inputCls = 'mt-1 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-500/20 transition';
	const errorCls = 'mt-1 text-xs text-red-600';
</script>

<!-- ─── Edit User Slide-Over ─────────────────────────────────────────────────── -->
{#if editUser}
	<div class="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm" onclick={() => editUser = null} role="presentation"></div>
	<div class="fixed right-0 top-0 z-50 flex h-full w-full flex-col bg-white shadow-2xl sm:max-w-md">
		<div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
			<div>
				<h2 class="text-lg font-semibold text-gray-900">Edit User</h2>
				<p class="text-xs text-gray-500">{editUser.email}</p>
			</div>
			<button onclick={() => editUser = null} class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition"><X size={20} /></button>
		</div>
		<div class="flex-1 overflow-y-auto px-6 py-6 space-y-4">
			<div class="grid gap-4 sm:grid-cols-2">
				<div>
					<label class="block text-sm font-medium text-gray-700">First Name</label>
					<input type="text" bind:value={editForm.firstName} class={inputCls} />
				</div>
				<div>
					<label class="block text-sm font-medium text-gray-700">Last Name</label>
					<input type="text" bind:value={editForm.lastName} class={inputCls} />
				</div>
			</div>
			<div>
				<label class="block text-sm font-medium text-gray-700">Role</label>
				<select bind:value={editForm.role} class={inputCls}>
			...
			{#if editForm.role === 'student'}
				<div class="grid gap-4 sm:grid-cols-2">
					<div>
						...
						<select bind:value={editForm.yearLevel} class={inputCls}>
					...
					<div>
						...
						<select bind:value={editForm.block} class={inputCls}>
							<option value="">Select...</option>
			...
			<div class="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 p-4">
				<div>
					...
					<p class="text-xs text-gray-500">{editForm.isActive ? 'User can access the system' : 'User is blocked from logging in'}</p>
				</div>
				<button
					type="button"
					onclick={() => editForm.isActive = !editForm.isActive}
					class="relative inline-flex h-6 w-11 flex-shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none {editForm.isActive ? 'bg-emerald-500' : 'bg-gray-300'}"
					role="switch" aria-checked={editForm.isActive}
				>
					<span class="inline-block h-5 w-5 rounded-full bg-white shadow transform transition-transform duration-200 {editForm.isActive ? 'translate-x-5' : 'translate-x-0'}"></span>
				</button>
			</div>
		</div>
		<div class="border-t border-gray-200 px-6 py-4 flex gap-3 justify-end">
			<button onclick={() => editUser = null} class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">Cancel</button>
			<button onclick={handleEditUser} disabled={editLoading} class="inline-flex items-center gap-2 rounded-lg bg-pink-600 px-4 py-2 text-sm font-semibold text-white hover:bg-pink-700 disabled:opacity-60 transition">
	...
	<div class="border-b border-gray-200">
		<nav class="-mb-px flex gap-1">
			{#each [['all','All Users'],['create','Create User'],['bulk-import','Bulk Import'],['inactive','Inactive Users']] as [tab, label]}
				<button
					onclick={() => { activeTab = tab as Tab; if (tab === 'all' || tab === 'inactive') loadUsers(); }}
					class="border-b-2 px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors {activeTab === tab ? 'border-pink-600 text-pink-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700'}"
				>{label}</button>
			{/each}
		</nav>
	</div>
	...
	{#if activeTab === 'all' || activeTab === 'inactive'}
		...
		{#if activeTab === 'all'}
			<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				<div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
					...
					<p class="mt-1 text-xs text-gray-400">{stats.total ? ((stats.active / stats.total) * 100).toFixed(1) : 0}% of total</p>
				</div>
				<div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
		...
		{#if activeTab === 'all'}
			<div class="flex flex-wrap gap-2">
				<span class="inline-flex items-center gap-1.5 rounded-full bg-blue-50 border border-blue-200 px-3 py-1 text-xs font-semibold text-blue-700">Students <span class="font-bold">{stats.students}</span></span>
				<span class="inline-flex items-center gap-1.5 rounded-full bg-violet-50 border border-violet-200 px-3 py-1 text-xs font-semibold text-violet-700">Instructors <span class="font-bold">{stats.instructors}</span></span>
				<span class="inline-flex items-center gap-1.5 rounded-full bg-pink-50 border border-pink-200 px-3 py-1 text-xs font-semibold text-pink-700">Custodians <span class="font-bold">{stats.custodians}</span></span>
			</div>
		{/if}
		...
		<div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
			<div class="flex flex-col gap-3 sm:flex-row sm:items-center">
				<div class="relative flex-1">
					...
					<input type="text" bind:value={searchQuery} oninput={onSearchInput} placeholder="Search by name or email…" class="w-full rounded-lg border border-gray-300 py-2 pl-9 pr-4 text-sm focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-500/20" />
				</div>
				<select bind:value={selectedRole} onchange={onFilterChange} class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-500/20">
				...
				{#if activeTab === 'all'}
					<select bind:value={selectedStatus} onchange={onFilterChange} class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-500/20">
				...
				<div class="flex gap-2">
					<button onclick={() => { bulkFile = null; bulkPreview = []; bulkErrors = []; activeTab = 'bulk-import'; }} class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
						<Upload size={16} />Import
					</button>
					<button onclick={() => activeTab = 'create'} class="inline-flex items-center gap-2 rounded-lg bg-pink-600 px-4 py-2 text-sm font-semibold text-white hover:bg-pink-700 transition shadow-sm">
			...
			{#if loading}
				<div class="flex items-center justify-center py-16">
					<div class="flex items-center gap-3 text-gray-500">
						<RefreshCw size={20} class="animate-spin text-pink-500" />
			...
			{:else}
					...
					<table class="w-full">
						<thead class="bg-gray-50 border-b border-gray-200">
							<tr>
								...
								<th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Class Info</th>
								<th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Last Login</th>
						...
						<tbody class="divide-y divide-gray-100">
							{#each users as user (user.id)}
								<tr class="hover:bg-gray-50 transition-colors group">
											...
											<div class="relative shrink-0 h-10 w-10">
												{#if getAvatarUrl(user)}
													<img
														...
														class="h-10 w-10 rounded-full object-cover ring-2 ring-white shadow-sm transition-opacity duration-300"
														onerror={(e) => {
													...
													/>
												{:else}
													<div class="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br {avatarColor(user.role)} text-sm font-bold text-white shadow-sm">
														{avatar(user)}
													</div>
												{/if}
											</div>
									...
									<td class="px-6 py-4">
										<span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {roleBadge(user.role)}">
											{user.role.charAt(0).toUpperCase() + user.role.slice(1)}
										</span>
									</td>
									<td class="px-6 py-4">
										<span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {statusBadge(user.isActive)}">
									...
									<td class="px-6 py-4 text-right">
										<div class="relative inline-block">
											<button
												onclick={(e) => { e.stopPropagation(); openDropdown = openDropdown === user.id ? null : user.id; }}
												class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-700 transition"
											><MoreVertical size={18} /></button>
											{#if openDropdown === user.id}
												<div class="absolute right-0 top-10 z-30 w-48 rounded-xl border border-gray-200 bg-white py-1 shadow-xl" onclick={(e) => e.stopPropagation()}>
													<button onclick={() => openEdit(user)} class="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition">
														<Edit size={15} class="text-gray-400" />Edit User
													</button>
													<button onclick={() => handleResetPassword(user)} class="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition">
														<KeyRound size={15} class="text-gray-400" />Reset Password
													</button>
													<div class="my-1 border-t border-gray-100"></div>
													<button onclick={() => handleToggleActive(user)} class="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm hover:bg-gray-50 transition {user.isActive ? 'text-amber-700' : 'text-emerald-700'}">
														{#if user.isActive}<UserX size={15} class="text-amber-500" />Deactivate{:else}<UserCheck size={15} class="text-emerald-500" />Reactivate{/if}
													</button>
													<button onclick={() => handleDelete(user)} class="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition">
														<Trash2 size={15} class="text-red-500" />Delete User
													</button>
												</div>
											{/if}
										</div>
								...
								<div class="flex items-center gap-3 min-w-0">
									...
									<div class="relative shrink-0 h-10 w-10">
										{#if getAvatarUrl(user)}
											...
											/>
										{:else}
											<div class="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br {avatarColor(user.role)} text-sm font-bold text-white">
												{avatar(user)}
											</div>
										{/if}
									</div>
									<div class="min-w-0">
										<p class="font-semibold text-gray-900 text-sm truncate">{user.firstName} {user.lastName}</p>
										<p class="text-xs text-gray-500 truncate">{user.email}</p>
									</div>
								</div>
								<div class="flex items-center gap-2 shrink-0">
									<span class="inline-flex rounded-full px-2 py-0.5 text-xs font-semibold {roleBadge(user.role)}">{user.role.charAt(0).toUpperCase() + user.role.slice(1)}</span>
									<button onclick={() => openEdit(user)} class="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 transition"><Edit size={16} /></button>
								</div>
							</div>
							<div class="mt-3 flex items-center gap-4 text-xs text-gray-500">
								<span class="inline-flex rounded-full px-2 py-0.5 font-semibold {statusBadge(user.isActive)}">{user.isActive ? 'Active' : 'Inactive'}</span>
				...
				{#if pagination.totalPages > 1}
					<div class="flex items-center justify-between border-t border-gray-200 px-6 py-4 bg-gray-50">
						...
						<div class="flex gap-2">
							<button
								onclick={() => { pagination.page -= 1; loadUsers(); }}
								disabled={pagination.page <= 1}
								class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-white disabled:opacity-40 transition"
							>Previous</button>
							<button
								onclick={() => { pagination.page += 1; loadUsers(); }}
	...
	{:else if activeTab === 'create'}
		<div class="mx-auto max-w-2xl">
			<div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
				...
				<form onsubmit={(e) => { e.preventDefault(); handleCreateUser(); }} class="mt-6 space-y-4">
					<div class="grid gap-4 sm:grid-cols-2">
						<div>
							<label class="block text-sm font-medium text-gray-700">First Name <span class="text-red-500">*</span></label>
							<input type="text" bind:value={createForm.firstName} class={inputCls} placeholder="Juan" />
							{#if createErrors.firstName}<p class={errorCls}>{createErrors.firstName}</p>{/if}
						</div>
						<div>
							<label class="block text-sm font-medium text-gray-700">Last Name <span class="text-red-500">*</span></label>
							<input type="text" bind:value={createForm.lastName} class={inputCls} placeholder="dela Cruz" />
					...
					<div>
						...
						<input type="email" bind:value={createForm.email} class={inputCls} placeholder="juan@example.com" />
					...
					<div>
						...
						<input type="password" bind:value={createForm.password} class={inputCls} placeholder="Min. 8 characters" />
					...
					<div>
						...
						<select bind:value={createForm.role} class={inputCls}>
					...
					{#if createForm.role === 'student'}
						<div class="rounded-xl border border-blue-100 bg-blue-50 p-4 space-y-4">
							...
							<div class="grid gap-4 sm:grid-cols-2">
								<div>
									<label class="block text-sm font-medium text-blue-800">Year Level <span class="text-red-500">*</span></label>
									<select bind:value={createForm.yearLevel} class={inputCls}>
								...
								<div>
									<label class="block text-sm font-medium text-blue-800">Block <span class="text-red-500">*</span></label>
									<select bind:value={createForm.block} class={inputCls}>
					...
					<div class="flex justify-end gap-3 border-t border-gray-200 pt-5">
						<button type="button" onclick={() => activeTab = 'all'} class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">Cancel</button>
						<button type="submit" disabled={createLoading} class="inline-flex items-center gap-2 rounded-lg bg-pink-600 px-5 py-2 text-sm font-semibold text-white hover:bg-pink-700 disabled:opacity-60 transition shadow-sm">
							{#if createLoading}<RefreshCw size={15} class="animate-spin" />{:else}<Plus size={15} />{/if}
							Create User
						</button>
					</div>
				</form>
	...
	{:else if activeTab === 'bulk-import'}
		<div class="mx-auto max-w-3xl space-y-6">
			...
			<div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
				...
				<div
					class="mt-6 rounded-xl border-2 border-dashed p-10 text-center transition-colors {isDragging ? 'border-pink-400 bg-pink-50' : 'border-gray-300 bg-gray-50 hover:border-pink-300 hover:bg-pink-50/50'}"
				...
				>
					<Upload size={40} class="mx-auto {isDragging ? 'text-pink-500' : 'text-gray-400'}" />
					<p class="mt-3 text-sm font-semibold text-gray-800">Drop your CSV file here, or</p>
					<label class="mt-3 inline-block cursor-pointer rounded-lg bg-pink-600 px-4 py-2 text-sm font-semibold text-white hover:bg-pink-700 transition shadow-sm">
						Browse File
						<input type="file" accept=".csv,.xlsx" class="sr-only" onchange={(e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (f) handleFileSelect(f); }} />
					</label>
					<p class="mt-2 text-xs text-gray-400">Supports .csv files · max 10 MB</p>
					{#if bulkFile}<p class="mt-2 text-sm font-medium text-pink-700">Selected: {bulkFile.name}</p>{/if}
				</div>

				<!-- Template -->
				<div class="mt-4 flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4">
					<Download size={18} class="mt-0.5 shrink-0 text-blue-600" />
					<div class="flex-1">
						<p class="text-sm font-semibold text-blue-900">CSV Template</p>
						<p class="mt-0.5 text-xs text-blue-700">Download the template to ensure correct column formatting</p>
						<p class="mt-1 font-mono text-xs text-blue-600">first_name, last_name, email, password, role, year_level, block</p>
					</div>
					<button onclick={downloadTemplate} class="shrink-0 rounded-lg border border-blue-300 bg-white px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-50 transition">
						Download
					</button>
				</div>
			</div>
			...
			{#if bulkPreview.length}
				<div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
					<div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
						<div>
							<h3 class="font-semibold text-gray-900">Preview — {bulkPreview.length} user{bulkPreview.length !== 1 ? 's' : ''} ready</h3>
							<p class="text-xs text-gray-500 mt-0.5">Review before importing</p>
						</div>
						<div class="flex gap-2">
							<button onclick={() => { bulkFile = null; bulkPreview = []; }} class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
								Clear
							</button>
							<button onclick={handleBulkImport} disabled={bulkImporting} class="inline-flex items-center gap-2 rounded-lg bg-pink-600 px-4 py-2 text-sm font-semibold text-white hover:bg-pink-700 disabled:opacity-60 transition shadow-sm">
								{#if bulkImporting}
									<RefreshCw size={15} class="animate-spin" />Importing {bulkProgress}%
								{:else}
					...
					{#if bulkImporting}
						<div class="px-6 py-3 bg-pink-50 border-b border-pink-100">
							<div class="flex items-center justify-between text-xs text-pink-700 mb-1.5">
								<span>Importing users…</span><span>{bulkProgress}%</span>
							</div>
							<div class="h-1.5 w-full rounded-full bg-pink-100 overflow-hidden">
								<div class="h-full rounded-full bg-pink-500 transition-all duration-300" style="width: {bulkProgress}%"></div>
							</div>
						</div>
					{/if}
							...
							<thead class="bg-gray-50">
								<tr>
									{#each Object.keys(bulkPreview[0]) as key}
										<th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">{key.replace(/_/g, ' ')}</th>
									{/each}
								</tr>
							</thead>
							<tbody class="divide-y divide-gray-100">
								{#each bulkPreview.slice(0, 10) as row}
									<tr class="hover:bg-gray-50">
										{#each Object.values(row) as val}
											<td class="px-4 py-3 text-gray-700 truncate max-w-[150px]">{val || '—'}</td>
										{/each}
									</tr>
								{/each}
							</tbody>
					...
					{#if bulkPreview.length > 10}
						<div class="border-t border-gray-200 px-6 py-3 bg-gray-50 text-center text-xs text-gray-500">
