<script lang="ts">
	...
	import { Moon, Sun, Bell, Type, Eye, RotateCcw } from 'lucide-svelte';
				...
				<div class="flex items-center justify-between px-6 py-4">
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<p class="mt-0.5 text-xs text-gray-500">Receive updates about support tickets and system events</p>
						</div>
					</div>
					<button
						onclick={() => toggleSetting('notificationsEnabled')}
						class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 {settings.notificationsEnabled ? 'bg-pink-600' : 'bg-gray-300'}"
						role="switch"
						aria-checked={settings.notificationsEnabled}
						aria-label="Toggle notifications"
					>
						<span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {settings.notificationsEnabled ? 'translate-x-6' : 'translate-x-1'}"></span>
					</button>
				</div>
			</div>
		</div>
