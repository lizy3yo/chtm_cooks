...
<svelte:head>
	<title>Notifications - Superadmin</title>
</svelte:head>

<div class="space-y-6">
	<div class="border-b border-gray-200 pb-5">
		<h1 class="text-3xl font-bold text-gray-900">Notifications</h1>
		<p class="mt-2 text-sm text-gray-600">Track support messages, system events, and user activity updates.</p>
	</div>
		...
		<div class="flex flex-col items-center justify-center rounded-xl border border-gray-200 bg-white p-16 text-center shadow-sm">
			...
			<p class="mt-2 max-w-sm text-sm text-gray-500">
				{filter === 'unread'
					...
					: 'Support messages and system events will appear here.'}
			</p>
		</div>
	{:else}
		<div class="space-y-3">
				...
				>
					<div class="flex items-start justify-between gap-4">
						<div class="flex items-start gap-4 min-w-0">
							<!-- Avatar -->
							<div class="mt-0.5 flex h-10 w-10 shrink-0 overflow-hidden items-center justify-center rounded-full text-sm font-semibold transition-colors {notification.isRead ? 'bg-gray-100 text-gray-500 group-hover:bg-gray-200' : 'bg-pink-100 text-pink-600 group-hover:bg-pink-200'}">
								{#if notification.metadata?.actorPhotoUrl || notification.metadata?.photoUrl}
									<img
										src={(notification.metadata?.actorPhotoUrl || notification.metadata?.photoUrl) as string}
										alt="User"
							...
							<div class="min-w-0 flex-1">
								...
								<p class="mt-1.5 text-sm leading-relaxed {notification.isRead ? 'text-gray-600' : 'text-gray-800'}">{notification.message}</p>
								<p class="mt-2.5 text-xs font-medium {notification.isRead ? 'text-gray-400' : 'text-pink-600/80'}">
