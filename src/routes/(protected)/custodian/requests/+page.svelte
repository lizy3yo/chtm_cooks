		...
		<div class="border-b border-gray-200">
			<nav class="-mb-px flex" aria-label="Tabs">
				<button
					...
					class="flex flex-1 items-center justify-center gap-1 border-b-2 px-1 py-3 text-[11px] font-medium whitespace-nowrap transition-colors sm:text-sm {activeTab ===
					'pending'
				...
				>
					...
					<span
						class="rounded-full px-1.5 py-0.5 text-[10px] {activeTab === 'pending'
				...
				>
					Ready
					<span
						class="rounded-full px-1.5 py-0.5 text-[10px] {activeTab === 'ready'
							? 'bg-pink-100 text-pink-600'
							: 'bg-gray-100 text-gray-600'}"
					>
						{tabCounts.ready}
		...
		<div class="rounded-xl bg-white shadow-sm">
			<div class="p-6">
				...
				{#if activeTab === 'history'}
					<div class="-mx-6 mb-6 border-b border-gray-200 px-6">
						<nav
						...
						>
							{#each [{ key: 'all', label: 'All', count: tabCounts.history }, { key: 'resolved', label: 'Resolved', count: tabCounts.historyResolved }, { key: 'completed', label: 'Completed', count: tabCounts.historyCompleted }, { key: 'cancelled', label: 'Cancelled', count: tabCounts.historyCancelled }] as sub}
								<button
									...
									class="flex flex-1 items-center justify-center gap-1 border-b-2 px-2 py-3 text-[11px] font-medium whitespace-nowrap transition-colors sm:flex-none sm:px-4 sm:text-sm {historySubTab ===
									sub.key
										? 'border-pink-500 text-pink-600'
										: 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700'}"
								>
									...
									<span
										class="shrink-0 rounded-full px-1.5 py-0.5 text-[10px] {historySubTab ===
										sub.key
											? 'bg-pink-100 text-pink-600'
											: 'bg-gray-100 text-gray-600'}"
									>
							...
							>
								...
								<div class="relative">
									...
									<svg class="pointer-events-none absolute inset-0 h-16 w-full" style="z-index: 0;">
										{#each [{ label: 'Request Submitted', by: 'Student', date: selectedRequest.requestDate, completed: true }, { label: 'Instructor Approved', by: selectedRequest.approvedBy, date: selectedRequest.approvedDate, completed: !!selectedRequest.approvedDate }, { label: 'Custodian Approved', by: 'Custodian', date: selectedRequest.releasedDate, completed: !!selectedRequest.releasedDate }, { label: 'Awaiting Pickup', by: 'Student', date: selectedRequest.pickedUpDate, completed: !!selectedRequest.pickedUpDate && selectedRequest.status !== 'ready' }] as step, idx}
											{@const stepCount = 4}
