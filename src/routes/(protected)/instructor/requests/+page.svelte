<script lang="ts">
	...
	const activeTabLoading = $derived.by(() => {
		switch (activeTab) {
			...
			case 'fulfillment': return fulfillmentLoading;
			case 'borrowed': return borrowedLoading;
	...
	async function bulkApprove(): Promise<void> {
		...
		try {
			const results = await Promise.allSettled(
				requestIds.map((rawId) => borrowRequestsAPI.approve(rawId))
			);
			let successCount = 0;
			let failureCount = 0;
			...
			if (failureCount > 0) {
				toastStore.error(
					`${failureCount} request${failureCount === 1 ? '' : 's'} failed to approve.`
				);
			}
	...
	async function bulkReject(): Promise<void> {
		...
		try {
			const results = await Promise.allSettled(
				requestIds.map((rawId) =>
					borrowRequestsAPI.reject(rawId, rejectReason, rejectDetails || undefined)
				)
			);
			let successCount = 0;
			let failureCount = 0;
			...
			if (failureCount > 0) {
				toastStore.error(
					`${failureCount} request${failureCount === 1 ? '' : 's'} failed to reject.`
				);
			}
	...
	function getStatusIconComponent(
		...
		if (status === 'fulfillment')
			return rawStatus === 'ready_for_pickup' ? CheckCircle2 : PackageCheck;
		if (status === 'borrowed') {
	...
	function getStatusColor(
		...
		if (status === 'fulfillment')
			return rawStatus === 'ready_for_pickup'
		...
		if (status === 'history') {
			...
			if (isCancelledRequest(rawStatus ?? 'returned', rejectionReason))
	...
	function getEmptyState(
		tab: 'pending' | 'fulfillment' | 'borrowed' | 'unresolved' | 'history',
		hasSearch: boolean
	) {
		if (hasSearch) {
			return {
				...
				description:
					'No student requests match your current search. Try a different name, request ID, or item keyword.'
			};
		}
		...
		return {
			...
			description:
				'Returned, resolved, cancelled, and rejected requests will appear here once the workflow is completed.'
		};
	}
</script>
...
<div class="space-y-6">
	...
	<div class="border-b border-gray-200">
		<nav class="-mb-px flex" aria-label="Tabs">
			{#each [{ key: 'pending', label: 'Pending', shortLabel: 'Pending', count: tabCounts.pending, urgent: false }, { key: 'fulfillment', label: 'Preparation', shortLabel: 'Prep', count: tabCounts.fulfillment, urgent: false }, { key: 'borrowed', label: 'Borrowed', shortLabel: 'Borrowed', count: tabCounts.borrowed, urgent: false }, { key: 'unresolved', label: 'Unresolved', shortLabel: 'Issues', count: tabCounts.unresolved, urgent: tabCounts.unresolved > 0 }, { key: 'history', label: 'History', shortLabel: 'History', count: tabCounts.history, urgent: false }] as tab}
				<button
					onclick={() => {
	...
	<div class="rounded-lg bg-white shadow">
		<div class="p-4 sm:p-6">
			...
			{#if activeTab === 'history'}
				<div
					class="-mx-4 -mt-4 mb-5 border-b border-gray-200 px-4 sm:-mx-6 sm:-mt-6 sm:mb-6 sm:px-6"
				>
					<nav
					...
					>
						{#each [{ key: 'all', label: 'All', count: tabCounts.history }, { key: 'resolved', label: 'Resolved', count: tabCounts.historyResolved }, { key: 'completed', label: 'Completed', count: tabCounts.historyCompleted }, { key: 'cancelled', label: 'Cancelled', count: tabCounts.historyCancelled }] as sub}
							<button
								onclick={() => (historySubTab = sub.key as typeof historySubTab)}
								class="flex flex-1 items-center justify-center gap-1 border-b-2 px-2 py-3 text-[11px] font-medium whitespace-nowrap transition-colors sm:flex-none sm:px-4 sm:text-sm
							...
							>
								...
								<span
									class="shrink-0 rounded-full px-1.5 py-0.5 text-[10px] {historySubTab === sub.key
										? 'bg-pink-100 text-pink-600'
										: 'bg-gray-100 text-gray-600'}">{sub.count}</span
								>
							</button>
						{/each}
					</nav>
			...
			<div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
				<div class="flex flex-1 items-center gap-2">
					...
					<div class="relative max-w-sm flex-1">
						...
						<input
							...
							class="block w-full rounded-lg border border-gray-300 bg-white py-2 pr-3 pl-9 text-sm focus:border-pink-500 focus:ring-1 focus:ring-pink-500 focus:outline-none"
						/>
					</div>
				</div>
				<div class="flex shrink-0 items-center gap-2">
					...
					{:else}
						<select
							bind:value={sortBy}
							class="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:border-pink-500 focus:ring-1 focus:ring-pink-500 focus:outline-none"
						>
									...
									{#if activeTab === 'pending'}
										<div class="absolute top-4 right-4 z-10">
											<input
												...
												onchange={(e) => {
													e.stopPropagation();
													toggleSelectRequest(request.rawId);
												}}
...
{#if showBulkRejectModal}
	<div class="fixed inset-0 z-50 overflow-y-auto">
		<button
			...
			onclick={() => (showBulkRejectModal = false)}
		...
		<div class="flex min-h-full items-center justify-center p-4">
			<div class="relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
				...
				<div class="mt-6 flex justify-end gap-3">
					...
					<button
						onclick={bulkReject}
						disabled={!rejectReason || bulkActionInFlight}
