<script lang="ts">
	...
	const faqs = [
		...
		},
		{
			...
			answer:
				'Switch to the "Chat Support" tab on this page to send a message directly to the system administrator. You will receive a reply in the same conversation thread.'
		}
	];
	...
	async function loadTickets() {
		...
		try {
			...
			toastStore.error('Could not load your support tickets. Please try again.');
		} finally {
	...
	$effect(() => {
		if (selectedTicket?.messages) scrollToBottom();
	});
...
<svelte:head>
	<title>Help & Support - Instructor Portal</title>
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="border-b border-gray-200">
		<nav class="-mb-px flex gap-6" aria-label="Support tabs">
			<button
				...
				class="flex items-center gap-2 border-b-2 pb-3 text-sm font-medium transition-colors {activeTab ===
				'faq'
			...
			<button
				...
				class="relative flex items-center gap-2 border-b-2 pb-3 text-sm font-medium transition-colors {activeTab ===
				'chat'
					? 'border-pink-500 text-pink-600'
					: 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700'}"
			>
				<MessageCircle size={16} />Chat Support
				{#if totalUnread > 0}<span
						class="flex h-5 min-w-5 items-center justify-center rounded-full bg-pink-500 px-1.5 text-[10px] font-bold text-white"
	...
	{#if activeTab === 'faq'}
		<div class="space-y-3">
			<div class="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-700">
				<strong>Can't find your answer?</strong> Switch to the
				<button
					onclick={() => (activeTab = 'chat')}
					class="font-semibold underline hover:text-blue-900">Chat Support</button
				> tab to send a message directly to the support team.
			</div>
			{#each faqs as faq, i}
				<div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
					...
					>
						...
						{#if openFaqIndex === i}<ChevronUp
								size={18}
								class="shrink-0 text-pink-500"
							/>{:else}<ChevronDown size={18} class="shrink-0 text-gray-400" />{/if}
					</button>
					{#if openFaqIndex === i}<div class="border-t border-gray-100 px-5 py-4">
	...
	{#if activeTab === 'chat'}
		<div
			class="flex h-[calc(100vh-20rem)] min-h-[560px] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm"
		>
			<!-- Left panel: conversation list -->
			<div
				class="flex w-full flex-col border-r border-gray-200 sm:w-72 lg:w-80 {mobileShowRight
					? 'hidden sm:flex'
					: 'flex'}"
			>
				<div class="flex items-center justify-between border-b border-gray-200 px-4 py-3">
					<div>
						...
						{#if tickets.length > 0}<p class="text-xs text-gray-400">
								{tickets.length} conversation{tickets.length !== 1 ? 's' : ''}
							</p>{/if}
					</div>
					<button
						onclick={resetToWelcome}
						class="flex items-center gap-1.5 rounded-lg bg-pink-500 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-pink-600"
					>
					...
					{:else}
						{#each tickets as ticket, i}
							...
							<div
								...
								class="group relative w-full cursor-pointer border-b border-gray-100 px-4 py-3 transition-colors hover:bg-gray-50 {isActive
									? 'bg-pink-50'
									: ''}"
							>
								<div class="flex items-start justify-between gap-2">
									...
									<button
										onclick={(e) => closeTicket(e, ticket)}
										class="shrink-0 rounded p-1 text-gray-300 opacity-0 transition-opacity group-hover:opacity-100 hover:text-red-400"
										aria-label="Close conversation"><Trash2 size={13} /></button
									>
								</div>
								<div class="mt-1.5 flex items-center justify-between gap-2">
									<span
										class="rounded-full px-2 py-0.5 text-[10px] font-semibold {statusTextColor(
											ticket.status
										)}">{statusLabel(ticket.status)}</span
									>
									<span class="text-[10px] text-gray-400">{formatDate(ticket.lastMessageAt)}</span>
								</div>
								{#if ticket.unreadByStudent > 0}<span
										class="absolute top-3 right-3 flex h-4 min-w-4 items-center justify-center rounded-full bg-pink-500 px-1 text-[9px] font-bold text-white"
										>{ticket.unreadByStudent}</span
									>{/if}
							</div>
						{/each}
					{/if}
			...
			<div class="flex flex-1 flex-col bg-white {mobileShowRight ? 'flex' : 'hidden sm:flex'}">
				{#if selectedTicket}
					...
					<div class="flex items-center gap-3 border-b border-gray-100 px-5 py-3">
						<button
							onclick={resetToWelcome}
							class="rounded-lg p-1 text-gray-400 transition-colors hover:bg-gray-100 sm:hidden"
							aria-label="Back"><X size={18} /></button
						>
						<div class="min-w-0 flex-1">
							<div class="flex items-center gap-2">
								<p class="text-sm font-bold text-gray-900">
									{ticketCode(tickets.findIndex((t) => t.id === selectedTicket!.id))}
								</p>
								<span
									class="rounded-full {statusBadgeColor(
										selectedTicket.status
									)} px-2.5 py-0.5 text-[10px] font-bold text-white"
					...
					<div
						...
						class="flex-1 space-y-4 overflow-y-auto bg-gray-50 px-5 py-4"
					>
						{#each selectedTicket.messages as msg}
							...
							<div class="flex {isMe ? 'justify-end' : 'justify-start'}">
								{#if isMe}
									<div class="max-w-[70%]">
										<div
											class="rounded-2xl rounded-br-sm bg-pink-500 px-4 py-2.5 text-sm text-white shadow-sm"
										>
								...
								{:else}
									<div class="max-w-[75%]">
										<div
											class="rounded-2xl rounded-bl-sm border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-800 shadow-sm"
										>
											<p
												class="mb-0.5 text-[11px] font-semibold {isAria
													? 'text-pink-500'
													: 'text-gray-700'}"
											>
						...
						{#if ariaThinking}
							<div class="flex justify-start">
								<div
									class="rounded-2xl rounded-bl-sm border border-gray-200 bg-white px-4 py-3 shadow-sm"
								>
						...
						{#if isClosed}
							<div class="flex justify-center py-2">
								<span class="rounded-full bg-gray-200 px-4 py-1.5 text-xs text-gray-500"
									>This conversation has been {selectedTicket.status}. Start a new chat if you need
					...
					<div class="border-t border-gray-100 bg-white px-4 py-3">
						<div
							class="flex items-center gap-2 rounded-full border {isClosed
								? 'border-gray-200 bg-gray-50'
								: 'border-gray-300 bg-white focus-within:border-pink-400 focus-within:ring-2 focus-within:ring-pink-100'}"
						>
							<input
								bind:this={inputEl}
								...
								disabled={isClosed || sendingMessage || ariaThinking}
							...
							<button
								...
								class="mr-1.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-colors {canSend &&
								!isClosed
									? 'bg-pink-500 text-white hover:bg-pink-600'
									: 'bg-gray-200 text-gray-400'} disabled:cursor-not-allowed"
								aria-label="Send"
							>
								{#if sendingMessage || ariaThinking}<LoaderCircle
										size={14}
										class="animate-spin"
									/>{:else}<Send size={14} />{/if}
							</button>
						</div>
				...
				{:else}
					...
					<div class="flex flex-1 flex-col items-center justify-between px-6 py-8">
						...
						<div class="w-full">
							<div
								class="flex items-center gap-2 rounded-full border border-pink-400 bg-white px-1 py-1 shadow-sm focus-within:ring-2 focus-within:ring-pink-100"
							>
								<input
									bind:this={inputEl}
									bind:value={messageInput}
									...
									disabled={sendingMessage}
									class="flex-1 bg-transparent px-3 py-1.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none"
								/>
								<button
									onclick={() => sendMessage()}
									disabled={!canSend}
									class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-colors {canSend
										? 'bg-pink-500 text-white hover:bg-pink-600'
										: 'bg-gray-200 text-gray-400'} disabled:cursor-not-allowed"
									aria-label="Send"
								>
									{#if sendingMessage}<LoaderCircle size={14} class="animate-spin" />{:else}<Send
											size={14}
										/>{/if}
								</button>
							</div>
